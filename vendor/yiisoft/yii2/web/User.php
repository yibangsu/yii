...
class User extends Component
{
    ...
    public $idParam = '__id';
    /**
     * @var string the session variable name used to store the value of [[authKey]].
     */
    public $usernameParam = '__username';
    /**
     * @var string the session variable name used to store the value of [[authKey]].
     */
    public $authKeyParam = '__authKey';
    /**
     * @var string the session variable name used to store the value of [[connection]].
     */
    public $connectionParam = '__connection';
    /**
    ...
    protected function beforeLogout($identity)
    {
        ...
        if ($connection !== null && $connection instanceof \yii\db\Connection)
        {
            try {
                $connection->close();
            } catch (Exception $e) {
    ...
    public function switchIdentity($identity, $duration = 0)
    {
        ...
        $session->remove($this->usernameParam);
        $session->remove($this->authKeyParam);
        $session->remove($this->connectionParam);
        $session->remove($this->authTimeoutParam);

        if ($identity) {
            $session->set($this->idParam, $identity->getId());
            $session->set($this->usernameParam, $identity->getUsername());
            $session->set($this->authKeyParam, $identity->getAuthKey());
            $session->set($this->connectionParam, $identity->getConnection());
     ...
     *
     * This method will try to determine the user identity using the [[idParam & authKeyParam]] session variable.
     *
    ...
    protected function renewAuthStatus()
    {
        ...
        $username = $session->getHasSessionId() || $session->getIsActive() ? $session->get($this->usernameParam) : null;
        $authKey = $session->getHasSessionId() || $session->getIsActive() ? $session->get($this->authKeyParam) : null;
        $connection = $session->getHasSessionId() || $session->getIsActive() ? $session->get($this->connectionParam) : null;

        if ($id === null || $authKey === null || $username === null || $connection === null) {
            $identity = null;
        } else {
            // @var $class IdentityInterface //
            $class = $this->identityClass;
            //$identity = $class::findIdentity($id);
            $identity = new $class($id, $username, $authKey, $connection);
        }
